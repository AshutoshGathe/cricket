...
/*-------------------------------------players stack for storing runs------------------------------*/
...
static void card_as(GtkWidget *button, gpointer window) {
	...
	/*while(i <= 11) {*/
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play1",  fourcount[1], sixcount[1], inscore[1], strikerate[1]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play2", fourcount[2], sixcount[2], inscore[2], strikerate[2]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play3",  fourcount[3], sixcount[3], inscore[3], strikerate[3]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play4",  fourcount[4], sixcount[4], inscore[4], strikerate[4]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play5",  fourcount[5], sixcount[5], inscore[5], strikerate[5]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play6",  fourcount[6], sixcount[6], inscore[6], strikerate[6]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play7",  fourcount[7], sixcount[i], inscore[7], strikerate[7]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play8",  fourcount[8], sixcount[8], inscore[8], strikerate[8]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play9",  fourcount[8], sixcount[i], inscore[8], strikerate[8]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play10",  fourcount[9], sixcount[9], inscore[9], strikerate[9]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play10",  fourcount[10], sixcount[10], inscore[10], strikerate[10]);
	fprintf(fp, "%s\t%d\t%d\t%d\t%.2f\n", "play11",fourcount[11], sixcount[11], inscore[11], strikerate[11]);

	fprintf(fp, "%s\t%d\t%d\n", "bowl1", overs[1], wickets[1]);
	fprintf(fp, "%s\t%d\t%d\n", "bowl2", overs[2], wickets[2]);
	fprintf(fp, "%s\t%d\t%d\n", "bowl3",  overs[3], wickets[3]);
	fprintf(fp, "%s\t%d\t%d\n", "bowl4", overs[4], wickets[4]);
	fprintf(fp, "%s\t%d\t%d\n", "bowl5", overs[5], wickets[5]);
	...
	label1 = gtk_label_new("TEAM 1\n\nName\t\tScore\tSix\tStrikeRate\n");
	gtk_box_pack_start(GTK_BOX(vbox), label1, 0, 0, 0);
	for(i = 1; i <= 12; i++) {		
		fgets(line, sizeof(line), file);
		label = gtk_label_new(line);
		gtk_box_pack_start(GTK_BOX(vbox), label, 0,0,0);
		totalscore = inscore[i] + totalscore; 
	}
	...
	for(i = 1; i <= 5; i++) {
		...
		label = gtk_label_new(line);
		gtk_box_pack_start(GTK_BOX(vbox), label, 0, 0, 0);
		fallwicket = wickets[i] + fallwicket;
	}
